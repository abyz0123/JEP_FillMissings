...
*************************
	*1. count missings	on sitation var 
				*************************		
	use "$datapath_jep/sharewX_rel7-1-0_gv_job_episodes_panel.dta", clear
...
*************************
	...
	*rename variables in wave 7 so that variable names of waves 3 and 7 are identical
		use "$datapath_w3/sharew3_rel7-1-0_re", replace
		 rename sl_* * //remove prefix 
				
		save "$datapath_w3/sharew3_rel7-1-0_re_renamed", replace
	
	use "$temp/imputation_temp", clear	
	merge m:1 mergeid using "$datapath_w3/sharew3_rel7-1-0_re_renamed", generate (_merge_sharelifew3) keep(match master)
	merge m:1 mergeid using "$datapath_w7/sharew7_rel7-1-1_re", generate (_merge_sharelifew7) update keep(master 3 4 5) keepusing(re*) 
